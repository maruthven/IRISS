...
/*
 *  Modified by Mike Vande Weghe, Carnegie Mellon University
 */
...
{
    nh.param("segwayOdomOriginTFName",segwayOdomOriginTFName, std::string("/segway_odom_origin"));
    nh.param("segwayOdomTFName", segwayOdomTFName, std::string("/segway_odom"));
    nh.param("segwayBaseTFName",segwayBaseTFName,std::string("segway_base"));
    nh.param("segwayLeftWheelTFName",segwayLeftWheelTFName,std::string("segway_wheel_left"));
    nh.param("segwayRightWheelTFName",segwayRightWheelTFName,std::string("segway_wheel_right"));

    odom.header.frame_id = segwayOdomOriginTFName;
    ...
    roSer = nh.advertiseService("ResetOdometry",&SegwayRMP::ResetOdometry,this);
    ...
    nh.param("max_xaccel", max_xaccel, 0.5); // meters/sec^2
    ...
    aa.setEulerZYX(0,0,-M_PI_2);
    bb.setEulerZYX(0,0,M_PI_2);
...
SegwayRMP::spin()
{
    ...
    while (nh.ok()) {  	
        ...
        if (ret == 1) {
            ...
            tf.sendTransform(tf::StampedTransform(tf::Transform(
                             ...
                             tf::Point(odom.pose.pose.position.x,
								...
								), odom.header.stamp, segwayOdomOriginTFName, segwayOdomTFName));
	    ...
	    right_wheel_rotations -= truncf(right_wheel_rotations);
            ...
            leftQuat.setEulerZYX(left_wheel_rotations * 2.0 * M_PI,0,0);
            rightQuat.setEulerZYX(-right_wheel_rotations * 2.0 * M_PI,0,0);
	    ...
	    tf.sendTransform(tf::StampedTransform (left_tf,ros::Time::now(),
                             segwayBaseTFName, segwayLeftWheelTFName));
	    tf.sendTransform(tf::StampedTransform (right_tf,ros::Time::now(),
                             segwayBaseTFName, segwayRightWheelTFName));
...
SegwayRMP::UpdateData(rmp_frame_t * data_frame)
{
    ...
    btQuaternion qt;
    qt.setEulerZYX(this->odom_yaw, pitch, roll);
...
double SegwayRMP::AccelLimit(double cmd_speed) {
  static double actual_speed=0;
  ...
  long elapsed_usec = (now.tv_sec * 1e6 + now.tv_usec) -
    (lasttime.tv_sec * 1e6 + lasttime.tv_usec);

  lasttime = now;

  if (cmd_speed == actual_speed) {
    return actual_speed;  // nothing to do
  }


  double max_delta_v = max_xaccel * elapsed_usec / 1.0e6;
  if (cmd_speed > actual_speed) {
    if (cmd_speed < (actual_speed + max_delta_v)) {
      actual_speed = cmd_speed;
    } else {
      actual_speed += max_delta_v;
    }
  } else {
    if (cmd_speed > (actual_speed - max_delta_v)) {
      actual_speed = cmd_speed;
...
SegwayRMP::MakeStatusCommand(CanPacket* pkt, uint16_t cmd, uint16_t val)
{
    ...
    int16_t trans = trans_speed_to_rmp_counts(AccelLimit(curr_xspeed));
    ...
    //
    /* Removed 10/15 by Mike Vande Weghe to try to correct
       assymetric turns (maybe the problem was unequal tire pressure
       or the second caster? */
    //
    //    if (rot > 0) {
    //        trans += 16;
    //    } else if (rot < 0) {
                               ...
                               double yawspeed)
{
    ...
    int16_t trans = trans_speed_to_rmp_counts(AccelLimit(xspeed));
...
bool SegwayRMP::ResetOdometry(pr_msgs::Idle::Request &req,
